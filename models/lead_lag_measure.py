...
def compute_lead_lag(data_subset, method, **kwargs):
    ...
    if method == 'ccf_at_lag':
        ...
        x = data_subset.iloc[:, 0].dropna().values.squeeze()
        y = data_subset.iloc[:, 1].dropna().values.squeeze()
    
        if x.ndim != 1 or y.ndim != 1:
            raise ValueError("DTW input must be 1-D vectors.")
    
        if len(x) < 3 or len(y) < 3:
            lead_lag_measure = np.nan
        else:
            forward_distance, _ = fastdtw(x, y, dist=euclidean)
            backward_distance, _ = fastdtw(y, x, dist=euclidean)
            lead_lag_measure = backward_distance - forward_distance
